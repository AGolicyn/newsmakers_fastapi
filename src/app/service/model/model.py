...
def trend_calculator(
    all_sources: list[str], trends: list[Trend], start_day: datetime.date, offset: int
) -> list[WeightTrend]:
    ...
    def _count_weights(self, k1: float, k2: float):
        ...
        return list(
            [
                ...
                for w1, w2, trend in zip(w1_weights, w2_weights, self.interval_trends)
            ]
        )
...
def make_trend_interval(
    trends: list[Trend], start_day: datetime.date, offset: int
) -> list[DayTrend]:
